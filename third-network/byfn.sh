...
function networkUp () {
  ...
  # 用这个方法把 script.sh 从 cli 的启动命令里分离出来
...
# 升级是一定要滚动重启网络的。先重启 orderer，再重启 peer。
...
function upgradeNetwork () {
  ...
  # docker cp 是把这个主机的这个目录拷贝到指定的位置上。-a是用 archive 模式。格式是 容器名:目录。
...
if [ "${MODE}" == "up" ]; then
  ...
  networkDown
  networkUp
# 整体而言，这个文件主要多出来这样一个函数，可以把1.0的 first-network 升级成1.1的first-network。
